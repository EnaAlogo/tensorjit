...
// modifications made :
// changed llvm::safe_malloc with std::malloc && bad_alloc throw if null check fails
// removed LLVM_ENABLE_EXCEPTIONS
// used std::realloc with null checking instead of llvm::safe_realloc
...
static_assert(sizeof(megu::SmallVector<void*, 0>) ==
    sizeof(unsigned) * 2 + sizeof(void*),
    "wasted space in SmallVector size 0");
static_assert(alignof(megu::SmallVector<Struct16B, 0>) >= alignof(Struct16B),
    "wrong alignment for 16-byte aligned T");
static_assert(alignof(megu::SmallVector<Struct32B, 0>) >= alignof(Struct32B),
    "wrong alignment for 32-byte aligned T");
static_assert(sizeof(megu::SmallVector<Struct16B, 0>) >= alignof(Struct16B),
    "missing padding for 16-byte aligned T");
static_assert(sizeof(megu::SmallVector<Struct32B, 0>) >= alignof(Struct32B),
    "missing padding for 32-byte aligned T");
static_assert(sizeof(megu::SmallVector<void*, 1>) ==
    sizeof(unsigned) * 2 + sizeof(void*) * 2,
    "wasted space in SmallVector size 1");

static_assert(sizeof(megu::SmallVector<char, 0>) ==
...
template class megu::SmallVectorBase<uint32_t>;
...
template class megu::SmallVectorBase<uint64_t>;
